...
Every **project** (an *upload* at the API and data-model level) has a single **main author**
— its owner.
...
1. **Request.** The current owner initiates a transfer, naming the target user. The project is
...
the project, before deciding whether to accept. This lets the prospective owner inspect the
...
pending transfer by listing their transfers through the API, or by noticing that the project
...
three state-changing steps — requesting, responding, and cancelling — require write access via
the `uploads:write` scope. A token without that scope can inspect transfers but cannot create,
accept, or cancel one.
...
See [How to transfer project ownership](../howto/manage/transfer_ownership.md) for the
