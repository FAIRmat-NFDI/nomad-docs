...
NOMAD supports multiple authentication mechanisms for API requests via [different tokens](TODO:).

Once a user has been identified, NOMAD determines the effective **scopes** that apply to the request, 
which describe the operations a user or token is allowed to perform. A scope defines a permission in the format:
...
## Token scopes (TODO: currently there might be some overlapping)
...
Scopes determine which API operations a user or token is allowed to perform.
...
which API operations the token is allowed to perform.

Unlike Keycloak access tokens, which typically have fixed and short lifetimes,
PATs can be created with a **customizable expiration time**. This makes them
well suited for long-running scripts or integrations where repeatedly acquiring
new tokens would be inconvenient.

PAT scopes must always be explicit scopes. Wildcard expressions such as
