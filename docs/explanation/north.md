...
[Since its addition to NOMAD as a service](https://joss.theoj.org/papers/10.21105/joss.05388), the backend and docker images behind NORTH tools have been in a process of significant refactoring. Not only did the docker base images evolve substantially but also most services of NOMAD were refactored into plugins. NORTH tools with demands for a graphical user interface that were initially based on [Webtop](https://docs.linuxserver.io/images/docker-webtop/) are becoming replaced by an image that is based on the [Jupyter Docker Stacks project](https://jupyter-docker-stacks.readthedocs.io/en/latest/using/selecting.html).

In this process, the storage location of the code that FAIRmat developed for NORTH changed. The [initial](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-remote-tools-hub) location had already been considered deprecated. **But this is not necessarily mentioned in all repos** The content in the location that was used [subsequently](https://gitlab.mpcdf.mpg.de/nomad-lab/north) is also in a process of refactoring whereby individual NORTH tools get anchored as a service to an **official NOMAD plugin (we need to distinguish these clean in the documentation, expectation management, and whose responsibility is it to maintain these?** that matches closest thematically and functionally.

## Defining official & community tools
**A definition ??? Official and community NORTH tools, the distinction should be documented; each official NORTH tool need to have a documentation and maintenance strategy, i.e., a responsible person from FAIRmat/NOMAD. We should add a section what community NORTH tool maintainers should do if they want to have their container listed here.**
**Will `nomad-north-` be a reserved prefix only for official containers ?

## Base images to build a tool

The constant refactoring that NORTH has experienced brought changes to locations of docker images and updates of the images. Relevant for a given NOMAD deployment is always the specific configuration YAML file that defines which NORTH tools are included. The starting point and example for such a configuration file is the respective [default.yaml file](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-FAIR/-/blob/develop/nomad/config/defaults.yaml) within the NOMAD main repository.

### nomad-north-desktop-base

[`nomad-north-desktop-base`](https://github.com/FAIRmat-NFDI/nomad-north-desktop-base), previously called [base-desktop](https://gitlab.mpcdf.mpg.de/nomad-lab/north/base-desktop/-/blob/main/Dockerfile?ref_type=heads), is the [Jupyter Docker Stacks project](https://jupyter-docker-stacks.readthedocs.io/en/latest/using/selecting.html) base docker image that we consider as the best practice to build NORTH tools from if these need support for GUI elements.

### jupyter-notebook
<!--### nomad-north-jupyter-base-->
[jupyter notebook] is the one as of 2026/01/19 shipped with nomad-FAIR. TODO
...
## Official tools (need functional check)
...
<!-- I do see also value in extending this container to a semantics playground e.g. connecting to the work for the nexus ontology, given that the NeXusOntology branch will soon be merged into pynxtools. One might also think about two containers, specifically here with a focus on FAIRmat II-->
...
## Need functional checks and additions
