---
hide: toc
---

# NOMAD How-to guides

How-to guides are goal-oriented instructions. Use them when you need to complete a specific task in NOMAD.

## Manage and find data

These guides cover day-to-day NOMAD usage for managing, exploring, and analyzing data.

<div markdown="block" class="home-grid">
<div markdown="block">

### With the GUI

- [Upload and publish data](manage/gui/upload.md)
- [Enter data with ELNs](manage/gui/eln.md)
- [Analyze data in NORTH](manage/gui/north.md)
- [Create custom workflows](manage/gui/workflows.md)
- [Write a YAML schema package](manage/gui/yaml.md)
- [Create custom ELNs](manage/gui/elns.md)
- [Parse tabular data](manage/gui/tabular.md)

</div>
<div markdown="block">

### Programmatically

- [API overview](manage/program/api.md)
- [Programmatic authentication](manage/program/auth.md)
- [Download data](manage/program/download.md)
- [Publish data using Python](manage/program/publish_python.md)
- [Access processed data](manage/program/archive_query.md)
- [Transform data](manage/program/json_transformer.md)
- [Make graph API calls](manage/program/graph_api.md)

</div>
</div>

## Develop plugins

These guides help advanced users and developers extend NOMAD with Python-based plugins.

- [Plugins overview](plugins/plugins.md)
- [Entry point types](plugins/types/actions.md)
- [Built-in plugin tools](plugins/tools/units.md)

## Host a NOMAD Oasis

These guides are for administrators running a NOMAD deployment for a lab or institution.

- [Install](oasis/install.md)
- [Configure](oasis/configure.md)
- [Deploy](oasis/deploy.md)
- [Update](oasis/update.md)
- [Administer](oasis/administer.md)
- [Secure](oasis/secure.md)
- [Troubleshoot](oasis/troubleshoot.md)

## Develop the core software

These guides are for contributors working on `nomad-lab`.

- [Get started](develop/setup.md)
- [Navigate the code](develop/code.md)
- [Contribute](develop/contrib.md)
- [Write a normalizer](develop/normalizing.md)
- [Extend the search](develop/search.md)
- [Make a release](develop/release.md)

