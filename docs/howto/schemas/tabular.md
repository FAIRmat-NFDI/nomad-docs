...
Refer to the [Reference guide](../../reference/annotations.md) for the full list of annotations connected to this parser and to the [Tutorials > Use NOMAD as an ELN > Parse tabular measurement data](../../tutorial/eln/tabular_parser_yaml.md)  for a detailed description of each of them.

## Preparing the tabular data file

NOMAD and `Excel` support multiple-sheets data manipulations and imports. Each quantity in the schema will be annotated with a source path composed by sheet name and column header. The path to be used with the tabular data displayed below would be `Sheet1/My header 1` and it would be placed it the `tabular` annotation, see [Reference > Annotations > Tabular data](../../reference/annotations.md#tabular-data).
...
Furthermore, we can insert comments before our data, we can use a special character to mark one or more rows as comment rows. The special character is annotated within the schema in the `tabular` annotation, see [Reference > Annotations > Tabular data](../../reference/annotations.md#tabular-data):
...
In this section eight examples will be presented, containing all the features available in tabular parser. Refer to the [Reference > Annotations > Available combinations](../../reference/annotations.md#available-combinations) for more comments on the implications of the structures generated by the following yaml files.
