...
Of course, if you add new information to the archive, this also needs to be defined in the schema (see [How-to guides > Work with schemas > Use base sections](../../schemas/advanced.md#extend-an-existing-section)). For example you could extend the section system with a special system definition that extends the existing section system definition:
