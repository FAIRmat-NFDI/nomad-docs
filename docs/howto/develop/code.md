...
There is one
[main NOMAD project](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-FAIR)
(and its [fork on GitHub](https://github.com/nomad-coe/nomad)).
...
[MatID](https://github.com/nomad-coe/matid),
...
!!! note
    ...
    [nomad-lab](https://gitlab.mpcdf.mpg.de/nomad-lab) and the GitHub
    organizations for [FAIRmat](https://github.com/fairmat-nfdi) and the
    [NOMAD CoE](https://github.com/nomad-coe) all represent larger
...
- `app`: The [FastAPI](https://fastapi.tiangolo.com/) APIs: v1 and v1.2
  NOMAD APIs,
...
- `cli`: The command line interface (based on
  [Click](https://click.palletsprojects.com)). Subcommands are
  structured into submodules.

- `config`: NOMAD is configured through the `nomad.yaml` file. This contains all the
  ([Pydantic](https://docs.pydantic.dev/)) models and default config
  parameters.
...
- `nomad/mkdocs.py`: Python code that defines
  [macros](https://mkdocs-macros-plugin.readthedocs.io/) which can be
  used in Markdown.
