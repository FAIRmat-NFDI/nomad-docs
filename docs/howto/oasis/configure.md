...
- The app, worker, and north service use the NOMAD docker image. Here we use the `latest` tag, which
  ...
  a version tag (format is `vX.X.X`, you find all releases at [nomad-FAIR > tags](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-FAIR/-/tags){:target="\_blank"}), or a specific [nomad-FAIR > branches](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-FAIR/-/branches){:target="\_blank"}.
