...
In this tutorial, we upload, share, and publish research data in NOMAD using the graphical user interface (GUI). We will use examples from both computational and experimental research. Step by step, we create uploads, manage access rights, add raw data files, review the entries generated by NOMAD, and organize selected entries into datasets. By the end of this tutorial, we publish the data and assign a Digital Object Identifier (DOI).
